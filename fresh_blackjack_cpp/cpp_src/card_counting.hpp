// cpp_src/card_counting.hpp
/*
 * Phase 2.3: Advanced Card Counting & Probability Engine
 * High-performance counting systems with real-time probability calculations
 */
...
struct CountState {
    ...
    CountState() : running_count(0), cards_seen(0), true_count(0.0),
                   advantage(0.0), penetration_percent(0) {}
};
...
struct ProbabilityResult {
    ...
    std::array<double, 22> dealer_total_probs;  // Index = total (0-21)
...
private:
    ...
    CountState state;
    DeckState deck;
    std::array<int, 10> cards_played;  // Track cards played by rank
...
public:
    ...
    // Probability calculations (cached for performance)
...
// Convert counting system enum to string
std::string counting_system_to_string(CountingSystem system);

// Get all available counting systems
